"""Python package to easy integration with Vodacom's Mpesa Public API.
"""
...
from pypesa.service_urls import Sandbox, Production
from pypesa.mpesa_exceptions import AuthenticationError, LoadingKeyError, MpesaConnectionError
...
class Mpesa:
    ...
    def authenticate(self) -> bool:
        ...
        Return True if the environment has auth keys else
    ...
    def load_keys(keys_filename: Union[str, Path]) -> Dict:
        ...
        Return keys Dict if auth file is present else,
    ...
    @environment.setter
    def environment(self, env: str) -> Union[Sandbox, Production]:
        ...
        """
        if isinstance(env):
            if env in ["Sandbox", "sandbox", "Production", "production"]:
                if env == "Sandbox" or "sandbox":
                    self.urls = Sandbox()
                elif env == "Production" or "production":
        ...
        raise TypeError(
            f"environment must be of type string not {type(env)}")
    ...
    def api_key(self, Api_key: str) -> str:
        """Use this property to explicit set a api_key
    ...
    def query_transaction_status(self, transaction_query: Dict) -> Dict:
        """Query transaction status method.
